...
class Report:
    ...
    def __createaYearSalary(self, job, dataFirst, dataSecond, a):
        titles = list(dataFirst.keys())
        avg = list(dataFirst.values())
        vacs = list(dataSecond.values())

        x = np.arange(len(titles))
        w = 0.35

        a.bar(x - w / 2, avg, w, label='средняя з/п')
        a.bar(x + w / 2, vacs, w, label=f"з/п {job}")

        a.set_title('Уровень зарплат по годам')
        a.set_xticks(x, titles, rotation=90)
    ...
    def __createCounterJobs(self, data, a):
        xa = list(data.values())
        xa.append(1 - sum(xa))
        cities = list(data.keys()) + ['Другие']

        a.set_title('Доля вакансий по городам')
        a.pie(xa, labels=cities, textprops={'fontsize': 6}, startangle=90)

    def createImage(self, data):
        f, ((a1, a2), (a3, a4)) = plt.subplots(nrows=2, ncols=2)

        self.__createaYearSalary(data[0], data[1], data[3], a1)
        self.__makeYearCounters(data[0], data[2], data[4], a2)
        self.__createCitySalary(data[5], a3)
        self.__createCounterJobs(data[6], a4)

        f.tight_layout()
        f.savefig('graph.png')

    def __makeYearCounters(self, job, dataFirst, dataSecond, a):
        titles = list(dataFirst.keys())
        counters = list(dataFirst.values())
        vacs = list(dataSecond.values())

        x = np.arange(len(titles))
        w = 0.35

        a.bar(x - w / 2, counters, w, label='Количество вакансий')
        a.bar(x + w / 2, vacs, w, label=f"Количество вакансий\n{job}")

        a.set_title('Количество вакансий по годам')
        a.set_xticks(x, titles, rotation=90)
        a.legend(prop={"size": 8})
        a.grid(axis='y')
        a.tick_params(axis='both', labelsize=8)

    def __createCitySalary(self, data, a):
        separator = lambda x: x.replace(' ', '\n').replace('-', '\n')

        cities = list(map(separator, data.keys()))[::-1]
        vals = list(data.values())[::-1]
        posY = np.arange(len(cities))

        a.barh(posY, vals)
        a.set_yticks(posY, labels=cities, fontsize=6)
        a.set_title('Уровень зарплат по городам')
        a.tick_params(axis='x', labelsize=8)

report = Report()
iConnection = InputConnection()
info = iConnection.dataInput()

report.generate_excel(info[:5], info[5:])
report.createImage(info)
